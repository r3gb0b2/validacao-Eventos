...
const App: React.FC = () => {
    ...
    // LÓGICA DE SINCRONIZAÇÃO AUTOMÁTICA (EXECUTA EM BACKGROUND)
    const runExternalSync = async (source: ImportSource, eventId: string) => {
        if (!db || !isOnline) return;
        try {
            console.log(`Auto-Sync: Iniciando ${source.name}...`);
            ...
            console.log(`Auto-Sync: ${source.name} finalizado (+${ticketsToSave.length}).`);
        } catch (e) { console.error(`Auto-Sync Error (${source.name}):`, e); }
    };
        ...
        <div className="min-h-screen bg-gray-900 text-white font-sans flex flex-col items-center p-4 md:p-8">
            <div className="w-full max-w-6xl mx-auto space-y-6">
                ...
                <main>
                    ...
                    {view === 'operators' && selectedEvent && <OperatorMonitor event={selectedEvent} scanHistory={scanHistory} isLoading={!ticketsLoaded} />}
