...
const App: React.FC = () => {
    ...
    const handleLogin = async (username: string, pass: string) => {
        ...
        try {
            ...
            snap.forEach(doc => {
                const data = doc.data();
                if ((data.username || '').toLowerCase() === normalizedInputName && data.password === pass) {
    ...
    const getAllowedEvents = () => {
        ...
        if (currentUser.role === 'SUPER_ADMIN' || currentUser.username === 'Administrador') return events; 
        const allowed = Array.isArray(currentUser.allowedEvents) ? currentUser.allowedEvents : [];
        return events.filter(e => allowed.includes(e.id));
    };
    ...
    }, [db, selectedEvent, ticketsMap, deviceId, operatorName, playBeep]);
        ...
        <div className="min-h-screen bg-gray-900 text-white font-sans flex flex-col items-center p-4 md:p-8">
            <div className="w-full max-w-6xl mx-auto space-y-6">
                ...
                <header className="flex justify-between items-center w-full">
                    {selectedEvent ? (
                        <div>
                            ...
                            <button onClick={() => setSelectedEvent(null)} className="text-sm text-gray-400 hover:underline">Trocar Evento</button>
                        </div>
                    ) : (
                        <div>
                            <h1 className="text-3xl font-bold text-orange-500">ST CHECK-IN</h1>
                        </div>
                    )}
                    <div className="flex items-center space-x-2">
                         ...
                         {currentUser && <button onClick={handleLogout} className="p-2 rounded-full bg-red-600 hover:bg-red-700 ml-2"><LogoutIcon className="w-6 h-6" /></button>}
                    </div>
                </header>
                ...
                <main>
                    ...
                    {view === 'public_stats' && selectedEvent && (
                        <PublicStatsView event={selectedEvent} allTickets={allTickets} scanHistory={scanHistory} sectorNames={sectorNames} hiddenSectors={hiddenSectors} isLoading={!ticketsLoaded} />
                    )}

                    {view === 'operators' && selectedEvent && (
                        <OperatorMonitor event={selectedEvent} scanHistory={scanHistory} isLoading={!ticketsLoaded} />
                    )}
                    ...
                    {view === 'admin' && (
                        <AdminView 
                            ...
                            isOnline={isOnline}
                            onSelectEvent={(e) => { setSelectedEvent(e); setView('admin'); }}
                    ...
                    {view === 'scanner' && !selectedEvent && !showLoginModal && (
                        <EventSelector events={events.filter(e => !e.isHidden)} onSelectEvent={(e) => { setSelectedEvent(e); setIsOperatorStep(true); }} onAccessAdmin={handleAdminRequest} />
                    )}
                </main>
            </div>
        </div>
