...
const AdminView: React.FC<AdminViewProps> = ({ db, events, selectedEvent, allTickets, scanHistory, sectorNames, hiddenSectors = [], onUpdateSectorNames, isOnline, onSelectEvent, currentUser, onUpdateCurrentUser }) => {
    ...
    // Carrega usuários quando a aba de eventos ou usuários está ativa e é Super Admin
    useEffect(() => {
    ...
    const handleToggleUserAccess = async (userId: string, eventId: string) => {
        ...
        const newAllowed = currentAllowed.includes(eventId)
            ? currentAllowed.filter(id => id !== eventId)
            : [...currentAllowed, eventId];
                                                            ...
                                                            <div className="grid grid-cols-1 gap-1 max-h-40 overflow-y-auto custom-scrollbar pr-1">
                                                                {allUsers.filter(u => u.role !== 'SUPER_ADMIN').map(u => (
                                                                    <label key={u.id} className="flex items-center p-1.5 rounded hover:bg-gray-700 cursor-pointer transition-colors">
                                                                        <input 
                                                                            type="checkbox" 
                                                                            checked={Array.isArray(u.allowedEvents) && u.allowedEvents.includes(ev.id)}
                                                                            onChange={() => handleToggleUserAccess(u.id, ev.id)}
                                                                            className="form-checkbox h-3.5 w-3.5 text-orange-600 rounded border-gray-600 bg-gray-900 focus:ring-0"
                                                                        />
                                                                        <span className="ml-2 text-xs font-medium text-gray-200">{u.username}</span>
                                                                    </label>
                                                                ))}
                                                                {allUsers.filter(u => u.role !== 'SUPER_ADMIN').length === 0 && (
                                                                    <p className="text-[10px] text-gray-500 italic">Crie usuários na aba "Usuários".</p>
                                                                )}
                                                            </div>
                                                            <button 
                                                                onClick={() => setAssigningEventId(null)}
                                                                className="w-full mt-2 text-[10px] text-gray-400 hover:text-white font-bold py-1 border border-gray-600 rounded uppercase"
                                                            >
                                                                Fechar
                                                            </button>
                                                        </div>
