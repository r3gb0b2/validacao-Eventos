...
const SettingsModule: React.FC<SettingsModuleProps> = ({ db, selectedEvent, sectorNames, hiddenSectors, importSources, onUpdateSectorNames, onUpdateImportSources, isLoading, setIsLoading, allTickets }) => {
    ...
    const handleToggleGlobalAutoImport = async () => {
        const newValue = !globalAutoImport;
        setGlobalAutoImport(newValue);
        await setDoc(doc(db, 'events', selectedEvent.id, 'settings', 'import_v2'), { 
            globalAutoImportEnabled: newValue 
        }, { merge: true });
    };
                ...
                <div className="space-y-6">
                    <div className={`p-6 rounded-3xl border transition-all shadow-xl flex items-center justify-between ${globalAutoImport ? 'bg-blue-600/10 border-blue-500' : 'bg-gray-800 border-gray-700'}`}>
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-sm font-black text-white uppercase tracking-widest">Sincronização Automática</h3>
                                <p className="text-[10px] text-gray-500 font-bold uppercase">Background Sync (Cloud)</p>
                            </div>
                        </div>
                        <button 
                            onClick={handleToggleGlobalAutoImport}
                            className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors focus:outline-none ${globalAutoImport ? 'bg-blue-600' : 'bg-gray-700'}`}
                        >
                            <span className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${globalAutoImport ? 'translate-x-7' : 'translate-x-1'}`} />
                        </button>
                    </div>
